...
export class UserEditComponent implements OnInit{
  ...
  async saveUser(userId: string | undefined) {
    ...
    try {
      ...
      const call = this.http.put<User>(environment.baseUrl + '/api/users/' + this.state.currentUser.userId, updatedData);
  ...
  async deleteUser(userId: string | undefined) {
    const call = this.http.delete(environment.baseUrl + '/api/users/' + userId);
  ...
  private async loadUserInfo(userId: number | undefined) {
    if (userId) {
      const call = this.http.get<User>(environment.baseUrl + '/api/users/' + userId);
